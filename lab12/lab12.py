...
def prune_min(t):
    ...
    if t.is_leaf():
        ...
        i = 0 if t.branches[0].label < t.branches[1].label else 1
        prune_min(t.branches[i])
        t.branches = [t.branches[i]]
...
def remainders_generator(m):
    ...
    """
    "*** YOUR CODE HERE ***"
    def gen_helper(remainder):
        n = 0 if remainder>0 else 1
        while True:
            yield n*m + remainder
            n += 1        
    return (gen_helper(i) for i in range(m))
