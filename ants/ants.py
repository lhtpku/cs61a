...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        if isinstance(self, LongThrower):
            while (place != beehive) and cnt<5:
                place = place.entrance
                cnt += 1
        ###
        while (place != beehive) and (not place.bees):
            place = place.entrance
            cnt += 1
        return random_or_none(place.bees) if (place!=beehive) and (self.min_range<=cnt<=self.max_range) else None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 5
        "*** YOUR CODE HERE ***"
        tmp = amount if amount<self.armor else amount+self.damage
        for bee in self.place.bees[:]:
            Insect.reduce_armor(bee,tmp)
...
class BodyguardAnt(Ant):
    ...
    def can_contain(self, other):
        # BEGIN Problem 9
        "*** YOUR CODE HERE ***"
        return (other.is_container==False) and (self.contained_ant is None)
        # END Problem 9
...
# END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if self.armor <= amount and self.order==0:
            bees_win()
        Ant.reduce_armor(self,amount)
        # END Problem 13
