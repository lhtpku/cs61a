...
class VendingMachine:
    ...
    def vend(self):
        if self.stock == 0:
            ...
            self.stock -= 1
            self.balance = 0
            return 'Here is your %s.' %self.name if change==0 else 'Here is your %s and $%s change.' %(self.name,change)
...
def generate_paths(t, x):
    ...
    if t.label == x:
        yield [x]
    for branch in t.branches:
        for tmp in generate_paths(branch, x):
            yield [t.label] + tmp
...
def remove_all(link , value):
    ...
    """
    "*** YOUR CODE HERE ***"
    ########## i solve this Q by the help of officous ans.
