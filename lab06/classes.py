...
class ProfessorCard(Card):
    ...
    def effect(self, other_card, player, opponent):
        ...
        "*** YOUR CODE HERE ***"
        opponent.deck.cards = [card for card in opponent.deck.cards if\
                                                    card.attack==self.attack and card.defense==self.defense]

        discarded = orig_opponent_deck_length - len(opponent.deck.cards)

        for i in range(len(player.deck.cards[:])):
            player.deck.cards[i] = Card(player.deck.cards[i].name, \
