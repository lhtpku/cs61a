...
def about(topic):
    ...
    "*** YOUR CODE HERE ***"
    def func(x):
        x = remove_punctuation(x).lower().split()
        return len(set(topic) & set(x)) > 0
    return func
    # END PROBLEM 2
...
def accuracy(typed, reference):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    a = len(typed_words); b = len(reference_words)
    if a*b==0:
        return 0.0
    res = 0
    for i in range(min(a,b)):
        res += typed_words[i]==reference_words[i]
    return 100*res/a
    # END PROBLEM 3
...
def edit_diff(start, goal, limit):
    ...
    if len(start)==0: # Fill in the condition
        ...
        # substitute_diff = swap_diff(start,goal,limit) 
        # # BEGIN
        # "*** YOUR CODE HERE ***"
