...
def distance(city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    tmp = (get_lat(city1)-get_lat(city2))**2 + (get_lon(city1)-get_lon(city2))**2
    return sqrt(tmp)
...
def closer_city(lat, lon, city1, city2):
    ...
    """
    "*** YOUR CODE HERE ***"
    virtual_point = make_city('stephen',lat,lon)
    return get_name(city1) if distance(virtual_point,city1)<=distance(virtual_point,city2) else get_name(city2)
...
def add_trees(t1, t2):
    ...
    if is_leaf(t1) and is_leaf(t2):
        ...
        b1 = branches(t1); b2 = branches(t2)
        if len(b1) >= len(b2):
            tmp = branches(t1)[len(b2):]
        else:
            tmp = branches(t2)[len(b1):]
        return tree(label(t1)+label(t2), [add_trees(b1,b2) for b1,b2 in zip(branches(t1),branches(t2))] + tmp)
